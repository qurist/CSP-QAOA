...
NINSTANCES = 100
Nvars      = 10#int(sys.argv[1])
p          = 2#int(sys.argv[2])

path = 'Random3Sat/n%d'%Nvars
csp.allocate(ppsi,Nvars)
...
for inst in range(NINSTANCES):
    csp.readcnf(path + '_%d.cnf'%inst, byref(H))
    csp.energize(pH)
    csp.uniform(ppsi)
    
    instances['Nvars'].append(H.n)
    instances['Nclauses'].append(H.m)
    instances['Cdensity'].append(H.n/H.m)
    PosIncidence= np.zeros(Nvars) # number of times a literal appears
    NegIncidence= np.zeros(Nvars) # number of times a negated literal appears
    PosIncidence= np.zeros(Nvars) # number of times a literal appears
    Incidence= np.zeros(Nvars) # total number of times a literal appears
    ...
    instances['Unsat'].append((Eopt - Emin)/H.m)
    csp.deallocateH(pH)
