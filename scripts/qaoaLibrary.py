...
scripts = '/home/aniruddha/Documents/Projects/QuICS/CSP/scripts'
#qc      = cdll.LoadLibrary('%s/lib2local-qaoa.so'%scripts)
csp     = cdll.LoadLibrary('%s/libcsp.so'%scripts)
...
def expectQAOAp_csp(psi, H, betagamma):
    ppsi = pointer(psi)
    pH   = pointer(H)
    csp.uniform(ppsi)
    p = len(betagamma)/2
...
# return the squared overlap with the known ground state indices
def overlapQAOAp_csp(psi, H, betagamma, sat):
    ppsi = pointer(psi)
    pH   = pointer(H)
    csp.uniform(ppsi)
    p = len(betagamma)/2
    ...
    overlap=0
    for i in sat:
        overlap += psi.realcur[i]*psi.realcur[i] + psi.imagcur[i]*psi.imagcur[i]
    return overlap

# Returns a list of ground state indices for a CSP hamiltonian
def ground_csp(H):
    n    = H.n
    pH   = pointer(H)
    Emin = 100
    # Compute minimum
    for i in range(1<<n):
        Emin = min(Emin, float(csp.energy(i,pH)))
    sat = []
    # Find minimizing states
    for i in range(1<<n):
        if csp.energy(i,pH)<=(Emin):
            sat.append(i)
    return (Emin, sat)
...
def optQAOA_csp(psi, H, pmax, sat, typeOfOpt='BFGS'):
    ppsi = pointer(psi)
    pH   = pointer(H)
#    fOpt = lambda bg: expectQAOAp_csp(psi, H, bg.tolist())
    fOpt = lambda bg: -overlapQAOAp_csp(psi, H, bg.tolist(), sat)
    bg0  = 0.5*np.ones(2)
    bgCur= bg0   
    for p in range(1,pmax+1):
        bgNew  = np.concatenate((bgCur[:p-1], [bgCur[p-1]], bgCur[p-1:], [bgCur[-1]]))
        opt    = minimize(fOpt, bg0 if p==1 else bgNew, method=typeOfOpt)
        bgCur  = opt.x
        E      = expectQAOAp_csp(psi, H, bgCur.tolist())
        #if E!=opt.fun: return "Error: Energy expectation does not match optimized value"
    return (bgCur, E) 

def optQAOAglobal_csp(psi, H, p, Nit=10, Temp=1.0, Tmax=20):
    ppsi = pointer(psi)
    pH   = pointer(H)
    fOpt = lambda bg: expectQAOAp_csp(psi, H, bg.tolist())
    bg0  = 2.0*np.ones(2*p)
    ...
    opt  = basinhopping(fOpt, bg0, niter=Nit, T=Temp, minimizer_kwargs=minKwargs, accept_test=stepCond)
    bgOpt= opt.x
    E      = expectQAOAp_csp(psi, H, bgOpt.tolist())
    return (opt.x, E)
...
def optQAOAgreedy_csp(psi, H, pmax, typeOfOpt='BFGS'):
    ppsi = pointer(psi)
    pH   = pointer(H)
    bg0  = [0.5]*2
    bgOpt  = []
    for p in range(pmax):
        fOpt   = lambda bg: expectQAOAp_csp(psi, H, np.concatenate((bgOpt,bg)).tolist())
        opt    = minimize(fOpt, bg0, method=typeOfOpt)
        bgOpt  += opt.x.tolist()
    E    = expectQAOAp_csp(psi, H, bgOpt)
    return (np.array(bgOpt), E) 

def optQAOAfull_csp(psi, H, p, M=10, trunc=-np.inf):
    S    = 2 # scale factor gamma:beta
    bMax = pi/2
    gMax = S*bMax
    indOpt= np.inf
